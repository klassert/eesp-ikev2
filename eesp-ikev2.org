...
- No extra setup time, a.k.a. zero round trip time to setup additional
 ...
 to reduce multiple SAs, Sub SA setup would be quicker.
...
- When using Hierarchical key derivation, the Sub SA key could be dervived
 on-the-fly, per packet for better performance, "Data-plane performance degradation due to the
 use of a larger number of keys", [[I-D.ponchon-ipsecme-anti-replay-subspaces]].

There are two types of Sub SAs, Session ID as: Replay Subspace ID,
specified in [[I-D.klassert-ipsecme-eesp]], and Sub SA with
independent keys.

# I don't think there are two types, we always need to encode
# the Replay Subspace ID on the Session ID.
# One kind is only splitting the SN, one set of keys
# Second one use per Sub SA keys.

To negotiate a Sub SA with with Replay Subspace,
use SUB_SA_SN in Session ID Transform.

To negotiate Sub SA with independent keys use transform,
SUB_SA_K in Session ID Transform.
...
The KEYMAT for the Sub SAs can be generated on the fly, for every packet,
when available memory is limited. This is usually the case when key derivation
