...
numbers when the Anti-Replay service is disabled.

This document specifies the negotiation of EESP Security
Associations (SAs) within the Internet Key Exchange Protocol Version
2 (IKEv2) framework [RFC7296]. It details the creation, rekeying,
and deletion of EESP SAs and the negotiation of associated transform
properties.

The extensions defined here enable EESP SAs to coexist with ESP SAs
when using stateful decryption, sharing a common SPI namespace
while introducing new capabilities that enhance IPsec’s
effectiveness in modern use cases. By starting with the SPI in the
Base Header, EESP maintains compatibility with existing ESP UDP
encapsulation formats, further simplifying integration.

Stateless implementations of EESP may only support either EESP or
ESP. The detailed use of stateless EESP, which is similar to
[[PSP]], is outside the scope of this document.
...
An EESP SA can be negotiate using IKEv2 in IKE_AUTH or
...
CREATE_CHILD_SA exchange. This negotiation the new  Security Protocol
Identifier value ~EESP~, as specified in this document, and uses the
