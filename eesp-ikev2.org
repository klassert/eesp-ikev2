...
#+RFC_ADD_AUTHOR: ("Tobias Brunner", "tobias@codelabs.ch", ("codelabs" "codelabs GmbH"))
#+RFC_AREA: SEC
#+RFC_WORKGROUP: IPSECME Working Group

#+begin_abstract
This document specfies how to negotiate Enhanced Encapsulating
...
This document specifies extensions to the Internet Key Exchange
Protocol Version 2 (IKEv2) [[RFC7296]] to negotiate an Enhanced
Encapsulating Security Payload (EESP) Child Security Association (SA)
as defined in [[I-D.klassert-ipsecme-eesp]]. An EESP SA is similar to
an ESP SA, as specified in [RFC4303], while introducing enhancements
to address evolving requirements in IPsec deployments.

This document describes how to establish, rekey, delete, and
negotiate the options of EESP SAs using the IKEv2 protocol.

The semantics of IKEv2 messages for EESP SAs are identical to those
for ESP SAs. All options and negotiation semantics applicable to
ESP also apply to EESP unless explicitly excluded.
...
** Terminology
It is assumed that readers are familiar with the  IKEv2 negotation
...
* EESP SA IKEv2 negotation
EESP IKEv2 is an extension to IKEv2 [[RFC7296]] to negotatiate the new
on EESP SA specified in [[I-D.klassert-ipsecme-eesp]].
An EESP SA can be negotatiated using IKEv2 in IKE_AUTH or
CREATE_CHILD_SA new SA, exchange.

IKEv2 Notify Message Status Type USE_WESP_MODE, [[RFC5840]], is not
supported when negotating EESP SA. As the WESP functionality
...
If this notification is received it should be discarded.

** New SA  using IKE_AUTH or CREATE_CHILD_SA exchange

An EESP Child SA can be negitationed using IKEv2 IKE_AUTH exchange
using Security Protolcol EESP specified in this document using EESP
Transform attric[[EESP SA Transforms]]


** Rekeying SA with CREATE_CHILD_SA Exchange

Rekeying an EESP is identical to rekeying an ESP SA as specified in
[[RFC7296]] section 1.3.3 and section 2.8  During rekey
[[EESP SA Transforms]] identical.

** Deleting SA with INFORMATIONAL Exchange

EESP SA always exist in pairs to delete an SA use IKEv2
INFORMATIONAL exchange as specified in [[RFC7296]] section 1.4.1.
...
EESP provides optional Anti-Replay Protection (ARP) using an
Extended Sequence Number (ESN) carried in the packet.
This feature can be negotiated using IKEv2 Transform Type 5,
Anti-Replay Protection [[ARP]]. When ARP is enabled using ESN, the full
64-bit ESN is included in each EESP packet. When present and
using EESP integerity protection the full 64bit ESN will be part of
integerity check.

When the Transform Type (ARP) is not present in initiaor's Child SA
proposl during negotiation of an EESP Child SA, the Sequence Number
field MUST NOT be transmitted in the EESP packet.

When ARP is not negotiated, i.e., when ESN is not carried in the
...
a single packet with different source addres or port number.
...
[[Anti-Replay Protection]] MUST be negotiated to carry a 64-bit ESN
in the EESP packet.

** EESP Version
EESB Base Header version specified [[I-D.klassert-ipsecme-eesp]]
...
Advantages of Sub SAs compared Child SAs with differnt keys

- Possiblity for unidirectional SA. Compared to [[RFC9611]] when per
  resourse SA is brought up it is bidirectional. However, both SA MAY
  not be in use. When using CREATE_CHILD_SA Unidirectal SAis not possible

- No extra setup time, a.k.a. zeroround trip time. Even though using
 IKE window size specified in [[RFC7296]] section 2.3 would aliviate
 setup this would be quicker.
- Creating Sub SA is more efficient while creating as well as rekeying
 and  deletinng, lifeicycle managment of Sub SA is simple.

*** Key derivation for Sub SA

[[RFC7296]] section 2.17 specifiys Child SA key generation.
When the EESP SA is negotiated with a Sub SAs, each SA need to derive
its own unique key. Sub SA allows to had it's of Sequence Number space or IV space.
This section specifys two methods for Sub SA key derivation.

Initally we are proposing two Key Derivation Functions for Sub SAs.
Based on community feedback, further reasearch and advise from
cryptographers choose one of the two.

**** Iterative key derivation
To iteratively derive keys create large keymt. e.g. for the nth Sub SAis
...
Where SK_d is derived from IKE negotation, as spectined in Section 2.14
of [[RFC7296]]
...
With exisitng prf+ function the keymat length rather limited.
[[RFC7296]] limit the iteration to 256.
However, with  modern prf+, more specifically XOF, functions,
such as KMAC spefified in [[NIST800-185 ]], or HopMAC/TurboSHAKE
specified [[I-D.irtf-cfrg-kangarootwelve]]  KEYMAT can be infinetly long.

Typical length of of 256 bit encryption is 36 bytes (32+4), in one direction.
Using an AEAD for 64K Sub SAs maximum KEYMAT length would be
...
Even though each Sub SA could be independently rekeyed, for the ease of use
when any one of the Sub SA need rekeying when reaching packet limited
all Sub SAs MUST be reakeyed  immediatly following the first rekey.
First replace the outgoing SAs. And incmoing SAs could be replace
whenever the peer received new Sub SA.

** Session ID

Session ID attribute is mutually exclusive multipuse attribute.

The Session ID attribute is a multi-purpose attribute with mutually
exclusive values. The initiator MUST propose one value in Child SA
proposal, and the responder MUST either agree with the
proposed value or respond with an INVALID_SESSION_ID message
indicating a supported value. The responder MUST
reject the request and indicate its preferred Session ID in
the INVALID_SESSION_ID Notify payload.

* UDP Encapsulation for EESP

UDP encapsulation is similar to ESP UDP encapsulation with one
diffrence on source port specified in [[RFC3948]] the EESP
allows to use diffrent source port than IKE as specified in
[[RFC3947]], [[RFC7296]] for Address and Port Agility and ECMP.
Sub SA may also use differernt source port while destination port 4500

* EESP Crypt Offset Option
This option is typically used for within one Datacenter use case
such as [[PSP]]. To negotatiate, the initiator sends USE_CRYPT_OFFSET
...
Several tables in [[IKEv2-IANA]] that specifiy ESP as protocol
...
requirements. For by adding Security Policy to the socket or route entry.
Also comply with ICMP  processing sepcified  in section 6 of RFC4301.
