...
To negotiate an EESP Child SA, use the IKEv2 IKE_AUTH or CREATE_CHILD_SA
exchange. This negotiation the new  Security Protocol Identifier value ~EESP~,
as specified in this document, and  uses the EESP Transform attributes
...
prorceduue as deleting Child SA using IKEv2 INFORMATIONAL exchange as
...
propse 1 (64 bit ESN) in Substructure of the Proposal Substructure
inside the Security Association (SA) payload in the IKEv2 Exchange.
When the responder select 64 bit ESN

This feature can be negotiated using IKEv2 Transform Type 5,
Sequence Number Properties [[SNP]]. When ARP is enabled using ESN,
the full 64-bit ESN is included in each EESP packet. When present and
using EESP integerity protection the full 64bit ESN will be part of
integerity check.

When the Transform Type SNP is not present in initiaor's Child SA
...
entry. Also comply with ICMP  processing sepcified  in section 6 of
RFC4301.
